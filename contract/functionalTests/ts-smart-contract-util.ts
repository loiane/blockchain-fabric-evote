...
export class SmartContractUtil {
    ...
    public static async getConnectionProfile() {
        ...
        const connectionProfilePath: string = path.join(homedir, '.fabric-vscode', 'environments', 'local_fabric', 'gateways', 'local_fabric.json');
    ...
    public static async submitTransaction(contractName: string, functionName: string, args: string[], gateway): Promise<Buffer> {
        ...
        if (contractName !== '') {
            contract = await network.getContract('voting', contractName);
        } else {
            contract = await network.getContract('voting');
        }
